...
export default function OpportunitiesPage() {
  ...
  // Agent handlers
  const handleOpenDetailsDialog = (opp: typeof opportunities[0]) => {
    setSelectedOpportunity(opp);
    ...
    <UnifiedLayout title={pageTitle}>
                ...
                <Card key={opp.id} className={`border-zinc-200 hover:border-teal-300 hover:shadow-lg transition-all overflow-hidden ${opp.status !== 'active' ? 'opacity-75' : ''}`}>
                  ...
                  <CardContent className="p-5">
                    ...
                    {isAdmin ? (
                      ...
                      <Button className="w-full bg-teal-500 hover:bg-teal-600" onClick={() => handleOpenDetailsDialog(opp)}>Apply<ArrowRight className="h-4 w-4 ml-2" /></Button>
                    ) : (
      ...
      <Dialog open={showDetailsDialog} onOpenChange={setShowDetailsDialog}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader><DialogTitle>{selectedOpportunity?.name}</DialogTitle><DialogDescription>{selectedOpportunity?.client}</DialogDescription></DialogHeader>
          {selectedOpportunity && (
            <div className="space-y-4 py-4">
              <p className="text-zinc-600">{selectedOpportunity.description}</p>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-emerald-50 rounded-lg p-4">
                  <div className="flex items-center gap-2 text-emerald-800 font-medium mb-2"><DollarSign className="h-4 w-4" />Compensation</div>
                  <p className="text-2xl font-bold text-emerald-900">${String((selectedOpportunity.compensation as Record<string, unknown>)?.baseRate || 0)}/hr</p>
                </div>
                <div className="bg-cyan-50 rounded-lg p-4">
                  <div className="flex items-center gap-2 text-cyan-800 font-medium mb-2"><GraduationCap className="h-4 w-4" />Training</div>
                  <p className="text-2xl font-bold text-cyan-900">{String((selectedOpportunity.training as Record<string, unknown>)?.duration || 0)} hours</p>
                </div>
              </div>
            </div>
          )}
          <DialogFooter>
            ...
            <Button onClick={handleApply} disabled={applying} className="bg-teal-500 hover:bg-teal-600">{applying ? 'Applying...' : 'Apply Now'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showSuccessDialog} onOpenChange={setShowSuccessDialog}>
        <DialogContent className="sm:max-w-md text-center">
          <div className="flex flex-col items-center py-6">
            <div className="w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center mb-4"><CheckCircle2 className="h-8 w-8 text-emerald-600" /></div>
            <DialogTitle>Application Submitted!</DialogTitle>
            <DialogDescription className="my-4">We'll notify you about the status.</DialogDescription>
            <div className="w-full bg-zinc-50 rounded-lg p-4 mb-4">
              <p className="text-xs text-zinc-500 mb-2">Application ID</p>
              <div className="flex items-center gap-2">
                <code className="flex-1 text-sm font-mono bg-white px-3 py-2 rounded border truncate">{applicationId}</code>
                <Button variant="outline" size="icon" onClick={copyApplicationId}>{copied ? <Check className="h-4 w-4 text-emerald-500" /> : <Copy className="h-4 w-4" />}</Button>
              </div>
            </div>
            <Button onClick={() => setShowSuccessDialog(false)} className="w-full bg-teal-500">Continue</Button>
          </div>
        </DialogContent>
      </Dialog>
    </UnifiedLayout>
